...
def main():
    ...
    try:
        ...
        if 'error' in eur_analysis:
            ...
            # AI分析结果 - 现在从technical_analysis中获取
        ...
        print(f"   🤖 AI分析功能: {'可用' if coordinator.analyzer.ai_enabled else '不可用'}")
...
def quick_test():
    ...
    try:
        ...
        if 'error' in analysis:
            ...
            print(f"价格: {analysis['latest_data']['price']:.4f}")
            ...
            if 'analysis' in ai_analysis:
                ...
                print(f"AI分析: {ai_analysis.get('warning', '未启用')}")
...
def test_ai_functionality():
    ...
    try:
        ...
        # 测试有AI分析
        print("\n1. 测试带AI分析:")
        analysis_with_ai = coordinator.analyze_currency_pair('EUR', 'USD', use_ai=True)
        technical_with_ai = analysis_with_ai['technical_analysis']
        
        if 'ai_analysis' in technical_with_ai:
            ai_result = technical_with_ai['ai_analysis']
            if 'analysis' in ai_result:
                print("✅ AI分析成功!")
                print(f"分析内容长度: {len(ai_result['analysis'])} 字符")
            else:
        ...
        # 测试无AI分析
        print("\n2. 测试无AI分析:")
        analysis_without_ai = coordinator.analyze_currency_pair('EUR', 'USD', use_ai=False)
        technical_without_ai = analysis_without_ai['technical_analysis']
        
        if 'ai_analysis' not in technical_without_ai:
            print("✅ 无AI分析模式工作正常")
        else:
            print("❌ 无AI分析模式异常")
